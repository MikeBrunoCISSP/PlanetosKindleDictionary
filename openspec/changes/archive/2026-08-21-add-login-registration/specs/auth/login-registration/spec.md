...
The system SHALL allow a registered user to authenticate using their email address and password. On success the system SHALL establish a session and return the user's profile.

#### Scenario: Successful login

- **WHEN** a POST request is sent to `/api/auth/login` with a valid `{ email, password }` matching a stored user
...
The system SHALL use HTTP-only, `SameSite=Lax`, signed cookies backed by Redis for session storage. Sessions SHALL NOT expose a session identifier in the JSON response body.
