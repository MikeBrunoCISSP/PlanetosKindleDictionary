 ...
 * correct target file.
 */
export function assignPlacements(entries) {
...
export function groupByLetter(placements) {
...
function needsLookupValue(entry) {
...
function rewriteCrossReferences(html, byId) {
    return html.replace(CROSS_REF_PATTERN, (match, targetId) => {
...
function renderEntry(placement, byId) {
    ...
    if (entry.partOfSpeech)
        metaParts.push(`<i>${escapeXmlText(entry.partOfSpeech)}</i>`);
    if (entry.pronunciation)
        metaParts.push(escapeXmlText(entry.pronunciation));
...
export function renderContentFile(placements, byId) {
